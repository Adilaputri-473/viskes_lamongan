...
<body class="hold-transition sidebar-mini layout-fixed" style="background-color: #f0fefc;">
    <div class="wrapper">
            ...
            <div class="visualisasi-section" id="dataCards">
                <div class="main-content">
                    ...
                    <div class="content-data" id="contentData">
                        <h2 class="text-center">Visualisasi Data Penyakit Menular</h2>

                        <div class="iframe-container mb-4 default-iframe">
                            <h3 class="text-center">Informasi dan Tren Pertahun</h3>
                            <iframe src="http://localhost:3000/public/dashboard/66db4fc9-5a5d-42ad-b34b-e58f8dc8c1d7"
                                width="100%" height="800px" frameborder="0" allowfullscreen>
                            </iframe>
                        </div>

                        <!-- Semua iframe berdasarkan pilihan -->
                        <div id="iframeContainer">
                            @foreach($visualisasi as $data)
                                <div class="iframe-container"
                                    data-tahun="{{ $data->tahun_id }}"
                                    data-penyakit="{{ $data->indikator_id }}"
                                    style="display: none;">
                                    <h3 class="text-center">{{ $data->penyakit_menular->indikator }} Tahun {{ $data->tahun->tahun }}</h3>
        ...
        <div class="sidebar-kanan-wrapper" id="sidebarKanan">
            <div class="sidebar-kanan">
                ...
                <p>Untuk melihat data berdasarkan tahun dan penyakit menular tertentu, atur pilihan pada dropdown berikut:</p>
                ...
                <label for="diseaseSelect">Penyakit</label>
                <select id="diseaseSelect" class="form-control">
                    <option value="">Pilih Penyakit</option>
                    @foreach($penyakit_menulars as $penyakit_menular)
                        <option value="{{ (string) $penyakit_menular->id }}">{{ $penyakit_menular->indikator }}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <script>
        ...
        function updateIframeVisibility() {
            ...
            const penyakit = diseaseSelect.value;
            ...
            iframes.forEach((container) => {
                ...
                const penyakitData = container.getAttribute('data-penyakit');

                if ((tahun === "" || tahunData === tahun) && (penyakit === "" || penyakitData === penyakit)) {
        ...
        diseaseSelect.addEventListener('change', updateIframeVisibility);
        ...
        function toggleSidebar() {
            ...
            if (sidebar.classList.contains('minimized')) {
                contentData.classList.add('full-width'); // hilangkan margin kanan
                toggleBtn.classList.add('minimized');
            } else {
                contentData.classList.remove('full-width'); // margin kanan sesuai sidebar
                toggleBtn.classList.remove('minimized');
            }
        }

        // Optional: set initial state sidebar shown
        // toggleSidebar(); // uncomment kalau mau dimulai sidebar minimize
